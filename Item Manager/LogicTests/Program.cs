...
namespace Item_Manager
{
    class Program
    {
        ...
        private static string GetBookData()
        {
            ...
            Console.Write("What's the author's first name? ");
            string authorFirst = Console.ReadLine();
            Console.Write("What's the author's last name? ");
            ...
            return $"{title}, {authorFirst}, {authorLast}, {pageCount}, {chapterCount}";
        }
        ...
        private static bool Execute()
        {
            ...
            switch (choice)
            {
                case "1":
                    ...
                    if (processor.Add(bookData[0], bookData[1], bookData[2], int.Parse(bookData[3]), int.Parse(bookData[4])) == true)
                    {
                ...
                case "2":
                    if (processor.ListAll().Count > 0)
                    {
                        ...
                        bookData = GetBookData().Split(',');
                        bool wasSuccessful = processor.Edit(bookData[0], bookData[1], bookData[2], int.Parse(bookData[3]), int.Parse(bookData[4]), index - 1);
                        if (wasSuccessful)
                        {
                ...
                case "3":
                    if (processor.ListAll().Count > 0)
                    {
                        int index = ChooseBookfromList(processor.ListAll());
                        bool exists;
                        Book localBook = processor.SearchByIndex(index - 1, out exists);
                        exists = !exists;
                        if (exists)
                        {
